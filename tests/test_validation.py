...
def _pane_fields_fast(_target, field):
    if "\t" in field:
        return "%1\tagent\t/project\t10"
    return _pane_fields(_target, field)
...
def test_configured_cursor_target_with_cursor_process_is_up(monkeypatch):
    ...
    monkeypatch.setattr(validation, "pane_field", _pane_fields_fast)
